...
(function () {
  ...
  function isPromptLike(text) {
    ...
    for (const ind of PROMPT_INDICATORS) {
      if (t.includes(ind.toLowerCase())) score++;
    }

    // 词数统计：英文按空格分，中文按字符算（2字≈1词）
    const enWords = text.split(/\s+/).filter(w => w.length > 0).length;
    const cnChars = (text.match(/[\u4e00-\u9fff]/g) || []).length;
    const effectiveWordCount = enWords + Math.floor(cnChars / 2);

    // 逗号统计：英文逗号 + 中文逗号
    const enCommas = (text.match(/,/g) || []).length;
    const cnCommas = (text.match(/，/g) || []).length;
    const totalCommas = enCommas + cnCommas;

    if (effectiveWordCount > 15 && totalCommas >= 3) score += 2;
    if (effectiveWordCount > 30) score += 1;
    ...
    // 中文长描述性文本（>100 中文字符，含多个中文逗号）
    if (cnChars > 100 && cnCommas >= 5) score += 2;
  ...
  function extractTags(text) {
    ...
    const tags = tagMap.filter(t => lower.includes(t.toLowerCase()));
